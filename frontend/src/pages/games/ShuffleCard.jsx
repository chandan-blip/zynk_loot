...
// Shuffle Card runs 4 parallel DURATION lanes. Each round reveals ONE card.
...
const SHUFFLE_MULTIPLIERS = { cards: 15, suit: 5, color: 2 };
...
function Clock3D({ size = 26, active = false, idKey = '' }) {
  const g = (s) => `shclock-${idKey}-${s}`;
  // Brushed-steel bezel works on both the lit and dark tab; gold accent
  // hands/ticks give Shuffle Card its casino identity.
  const faceTop = active ? '#1d2b3a' : '#16232e';
  const faceBot = '#0a1118';
  const hand = active ? '#fde68a' : '#facc15';
  const tick = active ? 'rgba(253,230,138,0.7)' : 'rgba(250,204,21,0.6)';
  return (
...
export default function ShuffleCard() {
  ...
  // ── Per-lane state (Shuffle Card runs 4 parallel duration lanes: 30s/1m/5m/10m) ──
  ...
  return (
    <div className="space-y-3">
          ...
          return (
            <button
              ...
              className={`group relative flex flex-col items-center justify-center gap-1.5 py-3.5 rounded-2xl border transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 ${
                active
                  ? 'border-gold/60 bg-gold/15 shadow-[0_6px_20px_-8px_rgba(234,179,8,0.6)] ring-1 ring-gold/40'
                  : 'border-dark-600/60 bg-dark-800/50 hover:border-gold/40 hover:bg-dark-700/50'
              }`}
            >
      ...
      <div
        className="relative overflow-hidden rounded-2xl border border-gold/25 bg-dark-800/60"
        style={{
      ...
      >
        ...
        <div
          ...
          style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(234,179,8,0.18) 0%, transparent 70%)' }}
        />
        ...
        <div className="relative p-4">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-2 min-w-0">
              <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-gold/15 border border-gold/30 shrink-0">
                <GiCardRandom className="w-5 h-5 text-gold-light" />
              </span>
              <div className="min-w-0">
                <h1 className="font-black text-base sm:text-lg leading-none truncate text-white tracking-tight">
                  Shuffle <span className="text-gold-light">Card</span>
                </h1>
            ...
            <button
              ...
              className="flex items-center gap-1 shrink-0 px-2 py-0.5 rounded-full border border-gold/40 bg-gold/10 text-gold-light hover:bg-gold/20 transition-colors text-[9px] font-bold uppercase tracking-wide"
            >
              ...
              return (
                <details key={b.betId} className="group">
                  ...
                  <summary className="flex items-center gap-2 px-3 py-2.5 cursor-pointer list-none hover:bg-dark-800/30">
                    ...
                    <span className="font-mono text-[11px] text-gold-light shrink-0">{formatPeriodId(b.periodId)}</span>
                    <span className="inline-block px-1.5 py-0.5 rounded text-[9px] font-black uppercase bg-gold/20 text-gold-light border border-gold/30 shrink-0">
                      {laneLabel}
                    </span>
        ...
        {howToOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-sm rounded-2xl border border-gold/30 bg-dark-800 shadow-2xl overflow-hidden"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-dark-600/50 bg-gradient-to-r from-gold/10 to-transparent">
                <div className="flex items-center gap-2">
                  <FiInfo className="w-4 h-4 text-gold-light" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="rounded-lg bg-dark-900/60 border border-dark-600/40 px-2 py-2 text-center">
                      <p className="text-[10px] text-gray-400">Color</p>
                      <p className="text-gold-light font-black text-sm">{SHUFFLE_MULTIPLIERS.color}x</p>
                      <p className="text-[9px] text-gray-500 mt-0.5">red / black</p>
                    </div>
                    <div className="rounded-lg bg-dark-900/60 border border-dark-600/40 px-2 py-2 text-center">
                      <p className="text-[10px] text-gray-400">Suit</p>
                      <p className="text-gold-light font-black text-sm">{SHUFFLE_MULTIPLIERS.suit}x</p>
                      <p className="text-[9px] text-gray-500 mt-0.5">♠♥♦♣</p>
                    </div>
                    <div className="rounded-lg bg-dark-900/60 border border-dark-600/40 px-2 py-2 text-center">
                      <p className="text-[10px] text-gray-400">Card</p>
                      <p className="text-gold-light font-black text-sm">{SHUFFLE_MULTIPLIERS.cards}x</p>
                      <p className="text-[9px] text-gray-500 mt-0.5">exact card</p>
                    </div>
                  </div>
                </div>
                <p className="text-[11px] text-gray-400 leading-relaxed">
                  Example: bet <b className="text-white">100</b> on a suit and hit — you win <b className="text-emerald-400">{100 * SHUFFLE_MULTIPLIERS.suit}</b>. Place as many slips as you like per lane; they all settle when the card reveals.
                </p>
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* Win/Loss overlay — uses the shared component so the modal animation
          and styling matches every other game (Shuffle Card, Coin Flip, etc.). */}
      <GameResultOverlay
        result={resultModal ? {
